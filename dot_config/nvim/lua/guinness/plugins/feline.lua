return {
  ...
  local feline = require('feline')
    ...
    local function maybe_invisible(color)
    ...
    table.insert(components.active[LEFT], {
      ...
      right_sep = 'slant_right',
      hl = {
        bg = maybe_invisible('sand'),
        fg = maybe_invisible('bg'),
      },
    })
    ...
    table.insert(components.active[RIGHT], {
      ...
      hl = {
        bg = maybe_invisible('violet'),
        fg = maybe_invisible('white'),
      },
    })
    ...
    hl = function()
        return {
          bg = maybe_invisible('blue'),  -- dynamically set based on filetype
          fg = maybe_invisible('white'), -- dynamically set based on filetype
        }
    end,
    })
...
-- insert the linenumber component at the end of the right section for inactive windows
...
-- insert the filename component after the mode component for inactive windows
table.insert(components.inactive[LEFT], {
  ...
  hl = function()
    return {
      bg = maybe_invisible('sand'),  -- dynamically set based on filetype
      fg = maybe_invisible('bg'),    -- dynamically set based on filetype
    }
  end,
})
